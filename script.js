...
class ProceduralMusicEngine {
    constructor() {
        ...
        this.styles = {
            ...
            random: {
                name: 'Random',
                bpmRange: [80, 140],
                chordProgressions: [], // Will be generated randomly
                bassPatterns: ['random'],
                arpPatterns: ['random'],
                drumPatterns: [0, 1, 2, 3, 4, 5]  // All patterns available
            }
        };
    ...
    generateBassPattern() {
        ...
        // Handle Random style differently - follow the chord progression
        if (this.currentStyle === 'random') {
            // Use stored progression (generated by pad pattern first)
            const chordProg = this.currentChordProgression || this.generateRandomProgression(8, 12);
            return this.generateBassFromChordProgression(chordProg, bassOctave);
        }
    ...
    // Generate bass pattern following chord progression (for Random style)
    generateBassFromChordProgression(chordProg, bassOctave) {
        const rhythm = this.bassRhythmOverride;
        const bassNotes = chordProg.map(degree => this.scaleNotes[degree % 7] + bassOctave);

        // Build pattern based on rhythm and chord progression
        let pattern = [];
        let subdivision = '8n';

        if (rhythm === '16th' || rhythm === '32nd') {
            // Fast rhythm - play each bass note with some repetition
            subdivision = rhythm === '16th' ? '16n' : '32n';
            const repsPerChord = rhythm === '16th' ? 4 : 8;

            bassNotes.forEach(note => {
                for (let i = 0; i < repsPerChord; i++) {
                    pattern.push(note);
                }
            });
        } else if (rhythm === '8th') {
            // 8th notes - play each bass note twice
            bassNotes.forEach(note => {
                pattern.push(note);
                pattern.push(note);
            });
            ...
            // Slower rhythms - one note per chord
            pattern = bassNotes;
            subdivision = '2n';
        }
    ...
    generatePadPattern() {
        ...
        let chordProg;
        if (this.currentStyle === 'random') {
            // Generate random progression with 8+ chords and store it
            chordProg = this.generateRandomProgression(8, 12);
            this.currentChordProgression = chordProg; // Store for bass to use
            console.log('[ProceduralEngine] Random progression (8-12 chords):', chordProg);
        } else if (this.useMarkovChain) {
    ...
    generateArpPattern() {
        ...
        if (arpPattern === 'arpeggiated' || arpPattern === 'fast-arpeggiated') {
            ...
            // Random: Generate 8 unique patterns using random scale notes
            return this.generateRandomArpPattern(scaleWithOctave, 16);
        } else {
    ...
    // Generate random chord progression (for Random style)
    generateRandomProgression(minLength = 8, maxLength = 12) {
        const length = this.randomInRange(minLength, maxLength);
        const progression = [];

        // Available scale degrees (0-6 for 7-note scale)
        const degrees = [0, 1, 2, 3, 4, 5, 6];

        for (let i = 0; i < length; i++) {
            // Randomly pick a scale degree
            const degree = this.randomChoice(degrees);
    ...
    // Generate random arp pattern (for Random style)
    generateRandomArpPattern(scaleWithOctave, length = 16) {
        const pattern = [];
        const density = Math.random(); // 0-1, determines how sparse/dense

        for (let i = 0; i < length; i++) {
            if (Math.random() < density) {
                // Add a note - pick random scale degree
                const degree = this.randomInRange(0, scaleWithOctave.length - 1);
                pattern.push(scaleWithOctave[degree]);
            } else {
                // Add silence
                pattern.push(null);
            }
        ...
        // Ensure at least 8 notes (not silences)
        const noteCount = pattern.filter(n => n !== null).length;
        if (noteCount < 8) {
            // Add more notes to reach minimum
            for (let i = 0; i < length && pattern.filter(n => n !== null).length < 8; i++) {
                if (pattern[i] === null) {
                    const degree = this.randomInRange(0, scaleWithOctave.length - 1);
                    pattern[i] = scaleWithOctave[degree];
                }
            }
        }
